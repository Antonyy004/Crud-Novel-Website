...
if (isset($_GET['hapus'])) {
    ...
    if ($conn->query($sql) === TRUE) {
        echo "<div class='alert alert-success' role='alert'>Akun berhasil dihapus!</div>";
    } else {
        echo "<div class='alert alert-danger' role='alert'>Error: " . $conn->error . "</div>";
    }
}
...
<div class="container">
    <h2 class="text-center">CRUD Akun Pengguna</h2>
    <!-- Form Tambah Akun -->
    <div class="card mb-3">
                ...
                <tbody>
                    <?php while ($row = $result->fetch_assoc()): ?>
                        <tr>
                            ...
                            <td>
                                <a href="edit_akun.php?id=<?= $row['id'] ?>" class="btn btn-warning btn-sm">Edit</a>
                                <a href="?hapus=<?= $row['id'] ?>" class="btn btn-danger btn-sm" onclick="return confirm('Apakah Anda yakin ingin menghapus akun ini?')">Hapus</a>
                            </td>
                        </tr>
                    <?php endwhile; ?>
                </tbody>
...
<!-- Footer -->
<footer class="footer">
    <p>&copy; 2025 BacaYuks | Semua hak cipta dilindungi</p>
</footer>
